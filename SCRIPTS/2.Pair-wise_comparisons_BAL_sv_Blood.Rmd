...
phenoData$tissue.source <- relevel(phenoData$tissue.source,"Blood")
```
...
for(i in grep("AM", unique(phenoData[,meta]),invert = T,value = T) ){
  out <- paste0("../ANALYSIS/PAIR-WISE_comparisons_BAL_vs_Blood/" )
  if(!dir.exists(out)){dir.create(out)}
  ...
  Group <- factor(phenoData2[,meta])
  design <- model.matrix(~ tissue.source, data = phenoData2)
  colnames(design)
...
# GSEA
```{r}
out2 <- paste0("../ANALYSIS/PAIR-WISE_comparisons_BAL_vs_Blood/" )
if(!dir.exists(out)){dir.create(out)}
