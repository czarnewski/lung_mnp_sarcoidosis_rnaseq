...
out <- paste0("/Users/paulo.barenco/Desktop/NBIS/Projects/A_SMED_SORENSEN_1805/ANALYSIS_COMPLETE/" )
...
phenoData$patient.cell <- paste0(phenoData$cell.type.ID,"_",phenoData$patient.type)
```
...
for(i in as.character(unique(phenoData[,meta])[-3]) ){
  out <- paste0("/Users/paulo.barenco/Desktop/NBIS/Projects/A_SMED_SORENSEN_1805/ANALYSIS_COMPLETE/NESTED_comparisons_Blood_vs_BAL_SP_vs_HP/" )
  if(!dir.exists(out)){dir.create(out)}
  ...
  design <- model.matrix(~ tissue.source * patient.type, data = phenoData2)
  colnames(design)
  #  Blood markers in SP = [ SP(Blood) - SP(BAL) ]  -  [ HC(Blood) - HC(BAL) ]
  contr <- c( 1,-1,-1,1)
  ...
  write.csv2(DEtable,paste0("DGE_list_",i,".csv") )
  write.csv2(DEtable[top_DE,],paste0("Signif_DGE_list_",i,".csv") )
  write.csv2(normcounts[top_DE,],paste0("NormCounts_",i,".csv") )
  ...
  pheatmap(normcounts[top_DE,order(phenoData2$tissue.source)],scale = "row",cluster_cols = F,color=colorRampPalette(c("navy","navy","white","firebrick3","firebrick3"))(30),
           clustering_method = "ward.D2",border=NA,cluster_rows = T,annotation_col = annotation_col, annotation_row = annotation_row, 
           annotation_colors = ann_colors,cutree_rows = 2,filename = paste0("Heatmap_DGE_",i,".pdf"))
  
  for(j in unique(cl)){  write.csv2(DEtable[top_DE[cl==j],],paste0("DGE_",i,"_gene_",j,".csv") ) }
  
  
  #Ploting volcano plot
  #png(filename = paste0("Volcano_plot_DGE_",i,".png"),width = 1000,height = 1000,res=200)
  pdf(paste0("Volcano_plot_DGE_",i,".pdf"),width = 6,height = 6, useDingbats = F)
  ...
  pdf(paste0("PieChart_DGE_",i,"_Chromossome_Representation.pdf"), useDingbats = F)
  ...
  for(k in unique(cl)){
    ...
    saveRDS(wc_kegg,file = "wc_go.rds")
    }
  }
  cl<-NULL
...
#Creating output directory
message("\nProcessing comparisson for # ",i," ...")
out <- paste0("/Users/paulo.barenco/Desktop/NBIS/Projects/A_SMED_SORENSEN_1805/ANALYSIS_COMPLETE/NESTED_comparisons_Blood_vs_BAL_SP_vs_HP/DGE_",i,"/" )
if(!dir.exists(out)){dir.create(out)}
setwd(out)
#-----------------------
...
for(k in GSEA_pathway_list){
  ...
  pathway <- gmtPathways(paste0("/Users/paulo.barenco/Desktop/NBIS/Projects/A_Smed_Sorensen_1805/SUPP_FILES/",k) )
...
lapply(wc_go,nrow)
names(wc_go)
unlist(lapply(wc_kegg,nrow))


pdf(paste0(out2,"GO_enrichment.pdf"),height = 6*3,width = 5)
par(mar=c(2,30,1,1),mfrow=c(par_list$n_mod[i],1))
for(k in names(wc_go)){
  if(dim(wc_go[[k]])[k]<5){n <- dim(wc_go[[k]])[1]}else{n<-5}
try(barplot(-log(wc_go[[k]][n:1,2],10),names.arg=wc_go[[k]][n:1,1],horiz=T,las=1,col=colorRampPalette(c("grey80", ifelse(k=="DOWN","navy","firebrick3")))(nrow(wc_go[[k]])),xlab=" ",xlim=c(0,max(-log(wc_go[[k]][n:1,2],10))*1.2),tck=-.05,border=NA,cex.axis=1.5,cex.names=1.5,main=paste0("GO  ",k)))
}
dev.off()

length(wc_go)
par(mar=c(2,20,1,1),mfrow=c(par_list$n_mod[i],1))
for(k in names(wc_go)){
  if(dim(wc_kegg[[k]])[1]<5){n <- dim(wc_kegg[[k]])[1]}else{n<-5}
try(barplot(-log(wc_kegg[[k]][n:1,2],10),names.arg=wc_kegg[[k]][n:1,1],horiz=T,las=1,col=colorRampPalette(c("grey80", ifelse(k=="DOWN","navy","firebrick3") ))(nrow(wc_kegg[[k]])),xlab=" ",xlim=c(0,max(-log(wc_kegg[[i]][n:1,2],10))*1.2),tck=-.05,border=NA,cex.axis=1.5,cex.names=1.5,main=paste0("KEGG  ",i)))
}
```
